...
PurePursuit::PurePursuit() : ld_(1.0), v_max_(5.0), v_(v_max_), w_max_(1.0), pos_tol_(0.1), idx_(0),
                             goal_reached_(false), nh_private_("~"), tf_listener_(tf_buffer_),
                             map_frame_id_("world"), robot_frame_id_("base_footprint"),
                             lookahead_frame_id_("lookahead")
{
  std::string frame_name = ros::this_node::getName() + "/robot_frame";
  nh_private_.param<string>(frame_name, robot_FRAME, "base_footprint");
  // Get parameters from the parameter server
  nh_private_.param<double>("wheelbase", L_, 1.0);
  nh_private_.param<double>("lookahead_distance", ld_, 10.0);
  ...
  nh_private_.param<string>("map_frame_id", map_frame_id_, "world");
  // Frame attached to midpoint of rear axle (for front-steered vehicles).
  nh_private_.param<string>("robot_frame_id", robot_frame_id_, robot_FRAME);
  ...
  sub_path_ = nh_.subscribe("target_path", 1, &PurePursuit::receivePath, this);
  sub_odom_ = nh_.subscribe("perfect_gps/utm_fixed", 1, &PurePursuit::computeVelocities, this);
...
void PurePursuit::computeVelocities(nav_msgs::Odometry odom)
{
  ...
  try
  {
    ...
    if (!goal_reached_)
    {
      ...
      double ld_2 = ld_ * ld_;
      double steering_cmd = std::min( 2*v_ / ld_2 * yt, w_max_ ); 
      cmd_vel_.angular.z = steering_cmd;
      ...
      cmd_vel_.linear.x = 10.0;
      cmd_acker_.drive.speed = 10.0;
